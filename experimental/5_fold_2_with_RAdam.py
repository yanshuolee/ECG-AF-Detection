...
def run(bs, path, lr, ks, num_layer):
    ...
    for X_train, Y_train, X_val, Y_val, val_cat in zip(training_data,
        ...
        validation_prediction = model.predict(X_val, batch_size=bs)
        score = f1_score(val_cat, np.argmax(validation_prediction, axis=1), average=None)
